...
- [x] 04-06-PLAN.md — macOS 真机人工验收 checkpoint:既有 Firefox 配置能力矩阵 / Gatekeeper 指引真实性 / 双语可读性 [UI-01..UI-04]
...
- [x] 05-01-PLAN.md — 资产入仓:`assets/app.icns` + dmg 拖拽背景图(含 @2x)[PKG-02/PKG-04]
- [x] 05-02-PLAN.md — `launch_app.py` macOS 运行时:Cmd+Q 汇入 force_exit + 首启 quarantine 自剥离与 D-12a 兜底提示 + 单测 [PKG-02/PKG-03]

**Wave 2** *(depends on 05-01)*

- [x] 05-03-PLAN.md — tracer:`build-macos` job 端到端(内核 → .app → plist → 注入 → 逐层签名 → 内外双验 → dmg → artifact)+ 首次真实 workflow_dispatch [PKG-01/02/03/04]
...
- [x] 05-05-PLAN.md — `release` 汇合 job 统一建 Release + Windows 发布步骤移出 build job(D-02 costly,含回归验证)[PKG-01/PKG-05]

**Wave 5** *(depends on 05-02 + 05-05;人工验收)*

- [x] 05-06-PLAN.md — macOS arm64 真机安装 checkpoint:首启提示序列 / quarantine 实测 / Cmd+Q / 图标观感 / Chrome 配置启动 [PKG-02/03/04]
...
**Success Criteria** (what must be TRUE):
  ...
  3. 在开发者现有账户中(`/Applications` 已预先卸载此前安装),仅针对 arm64 验证:验证者只依据发布文档记载的内容完成下载、放行、安装、创建配置并启动 Chrome 配置的完整流程,不得使用文档之外的已有经验(D-15)。**备选放行路径「系统设置 → 隐私与安全性 → 仍要打开」不在本次验收范围内,记为未验证**(见下方 2026-07-31 澄清)
...
- [x] 06-03-PLAN.md — 另外两个消费面对齐实测流程:应用内 `gatekeeper.step1`–`step4` 中英改写 + 顺序锁单测 + dmg 背景图重生成 [DOCS-01]
