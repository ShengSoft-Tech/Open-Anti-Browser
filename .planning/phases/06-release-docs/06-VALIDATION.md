...
| 06-05/Task3 | 06-05 | Wave 4 | DOCS-01, DOCS-02 | T-06-12 | Release notes 渲染出带引号命令、递进三步、前置要求清单 | manual_procedural | 无自动化等价物 — D-13/D-15 人工验证;06-05 Task3 执行 | ✅ | ⚠️ flaky — approved with narrowed scope: developer approval covers primary path (再双击一次)/自查/安装/创建并启动 Chrome 配置四段;D-13 干净账户要求经开发者豁免,备选路径「系统设置 → 仍要打开」记为未验证(该账户已带 Phase 5 写入的 Gatekeeper denial breadcrumb);B1 首启弹窗逐字文案仍未捕获,记为遗留项。详见 06-05-SUMMARY.md |
| 06-05/Task2 | 06-05 | Wave 4 | DOCS-02 | T-06-09 | dmg 背景图文案匹配实测首启流程,且不含 `spctl` / `sudo` / `--master-disable` / 宽目录递归 | manual_procedural | 源 HTML 禁用词 grep + 目视核对(照搬 05-01 的 Forbidden-phrase audit;06-03 已跑过一次针对 06-03 内容自身的 scratch HTML 审计) | ✅ | ✅ green — 真实 v0.2.0 dmg 挂载安装(06-05 Task3 走完拖拽安装步骤)未报告背景图/图标错位;禁用词片段扫描(sudo/spctl/--master-disable/csrutil/~/Downloads)已对已发布 Release 正文核对为 0 命中,见 06-05-SUMMARY.md acceptance criteria checks。未做逐像素独立复核 |
